...
```{r RF determine number of parameters, fig.width=6, fig.height=4}
...
aov_meth <- aov(lm(RC_annual ~ Partition_group, data = allData2)) 
TukeyHSD(aov_meth)
...
allData2 %>% 
  ...
  labs(x = element_blank(),
       y = element_blank()) +
  annotate("text", x = 792, y = 0.56, label = "*", col = "black", hjust = 0) +
  annotate("text", x = 826, y = 0.73, label = "*", col = "black", hjust = 0) +
  annotate("text", x = 852, y = 0.60, label = "*", col = "black", hjust = 0) ->
  factor_test2
...
aov_cli <- aov(lm(RC_annual ~ MiddleClimate, data = allData2 %>% filter(MiddleClimate %!in% c("A")))) 
TukeyHSD(aov_cli)
allData2 %>% 
  dplyr::select(RC_annual, Rs_annual, MiddleClimate) %>% 
...
Anova(lm(RC_annual ~ Ecosystem_type, data = allData2 %>% filter(Ecosystem_type %!in% c("Urban", "Desert", "Savanna", "Tundra"))), type="III") 
aov_eco <- aov(lm(RC_annual ~ Ecosystem_type, data = allData2 %>% filter(Ecosystem_type %!in% c("Urban", "Desert", "Savanna", "Tundra")))) 
TukeyHSD(aov_eco)
allData2 %>% 
  dplyr::select(RC_annual, Rs_annual, Ecosystem_type) %>% 
  filter(Ecosystem_type %!in% c("Urban", "Desert", "Savanna", "Tundra")) %>%
  ...
  labs(x = expression(Annual~R[S]~(g~C~m^{-2}~yr^{-1})),
       y = element_blank()) +
  annotate("text", x = 437, y = 0.45, label = "*", col = "black", hjust = 0) +
  annotate("text", x = 830, y = 0.73, label = "*", col = "black", hjust = 0) ->
  factor_test4
```
