...
# panel b
tibble(x = c("ER%", "NM%", "EM%", "Climate", "AM%", "BMa", "N dep", "Clay%", "MAT", "SOC", "BMb", "EVI", "BD", "MAP"),
...
# panel c
tibble(x = c("ER%", "NM%", "EM%", "Climate", "AM%", "Clay%", "N dep", "SOC", "BMb", "BMa", "MAT", "EVI", "BD", "MAP"),
...
# test ecosystem
Anova(lm(RC_annual ~ Ecosystem2, data = allData2), type="III") 
allData2 %>% 
  dplyr::select(RC_annual, Rs_annual, Ecosystem2) %>% 
  filter(Ecosystem2 %!in% c("A")) %>% 
  group_by(Ecosystem2) %>% 
  summarise(RC_mean = mean(RC_annual), Rs_mean = mean(Rs_annual),
            RC_sd = sd(RC_annual), Rs_sd = sd(Rs_annual), obs = n()) %>% 
  ggplot(aes(Rs_mean, RC_mean, col = Ecosystem2)) +
  geom_point(alpha = 1, aes(size = obs), show.legend = FALSE) +
  geom_errorbar(aes(ymin = RC_mean - RC_sd, ymax = RC_mean + RC_sd)) +
  # stat_function(fun = function(x) { 0.082 + 0.05*log(x) }, col = 'pink', size = 1.5) +
  # geom_smooth(method = "lm", col = "black", se = FALSE) +
